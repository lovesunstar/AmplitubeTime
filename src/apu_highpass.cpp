...
//
HighPassFilter::HighPassFilter( QWidget * parent ) 
	: LinearFilter(),
	...
	m_disabled_icon(":/images/off.png")
{
	...
	QLabel * title_label = new QLabel(tr("<b>High-pass filter</b>"));
...
//
QString HighPassFilter::name()
{
	return tr("High-pass filter");
}


void HighPassFilter::recalcCoeffs()
{
	m_order = 2;
...
void HighPassFilter::setCutoff(int cutoff)
{
	if(cutoff != m_cutoff){
		m_cutoff = cutoff;
		recalcCoeffs();
...
void HighPassFilter::setFreq(int freq)
{
	if(freq != m_freq){
		m_freq = freq;
...
void HighPassFilter::bypassSwitch()
{
	m_bypass = !m_bypass;
...
void HighPassFilter::parametersChanged(int freq)
{
	m_cutoff = freq;
	m_cutoff_spin->setValue(m_cutoff);
	recalcCoeffs();
...
void HighPassFilter::writeSettings(QSettings & s)
{
	s.setValue("cutoff", m_cutoff);
}

void HighPassFilter::readSettings(QSettings & s)
{
	qDebug() << "HighPassFilter::readSettings";
